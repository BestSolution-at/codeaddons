package at.bestsolution.asciidocj;
...
public class Activator implements BundleActivator {
	...
	private void setupRubyGems(Bundle b) {
		...
		// as path.
		String path = explodeEntries(b, "gems", "/libs/asciidoctorj-1.5.2.jar!cache",
				"/libs/asciidoctorj-1.5.2.jar!gems", "/libs/asciidoctorj-1.5.2.jar!specifications",
				"/libs/asciidoctorj-1.5.2.jar!bin");
		...
		// We don't have access to that Ruby runtime, so we'll have to do it via
		// the JVM's
		// environment.
	...
	private static void forceEnv(String envVarName, String envVarValue) {
		...
		// We don't want the UnmodifiableMap, we want the underlying
		// (modifiable) map
	...
	private static String explodeEntries(Bundle b, String home, String... paths) {
		File homeDir = b.getDataFile(home);
		if (!homeDir.exists()) {
			homeDir.mkdir();
		}
		// For performance reasons, you'll probably want to store some sort of
		// token in
		// the data dir to indicate that you've already extracted the files, and
		// don't
		// need to read the jar again. As written, we'll scan the asciidoctor
		// jar every
						...
						if (file.createNewFile()) {
							...
							while (bytesRead < bytesToRead) {
								...
								if (bytesLeft < bufferSize) {
									readSize = (int) bytesLeft; // less than
																// bufferSize,
																// so must fit
																// in an int
								}
